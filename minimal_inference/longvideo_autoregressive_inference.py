...
parser.add_argument("--config_path", type=str, default="configs/wan_causal_dmd.yaml")
...
parser.add_argument("--output_folder", type=str, default="experiments/ar_long")
...
for prompt_index in tqdm(range(len(dataset))):
    ...
    export_to_video(
        video, os.path.join(args.output_folder, f"long_video_output_{prompts[0][:50]}.mp4"), fps=16)
