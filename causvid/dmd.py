...
class DMD(nn.Module):
    def __init__(self, args, device):
        ...
        self.real_score = get_diffusion_wrapper(
            model_name=self.real_model_name)(type = args.real_score_type)
        self.real_score.set_module_grad(
            module_grad=args.real_score_grad # 14B / 1.3 finetuned sketchs
        )

        self.fake_score = get_diffusion_wrapper(
            model_name=self.fake_model_name)(type = args.fake_score_type)
        self.fake_score.set_module_grad(
            module_grad=args.fake_score_grad  # 1.3B finetuned sketchs
        )

        # load real and fake score ckpts if applicable
        if getattr(args, "real_score_ckpt", False):
            print(f"Loading pretrained real score from {args.real_score_ckpt}")
            # state_dict = torch.load(args.real_score_ckpt, map_location="cpu", weights_only=False)[
            #     'real_score']
            state_dict = safetensors.torch.load_file(args.real_score_ckpt)

            self.real_score.model.load_state_dict(
                state_dict, strict=True
            )

        if getattr(args, "fake_score_ckpt", False):
            print(f"Loading pretrained fake score from {args.fake_score_ckpt}")
            # state_dict = torch.load(args.fake_score_ckpt, map_location="cpu", weights_only=False)[
            #     'fake_score']

            state_dict = safetensors.torch.load_file(args.fake_score_ckpt)

            self.fake_score.model.load_state_dict(
                state_dict, strict=True
            )

        # === LoRA 支持 for real_score ===
        if getattr(args, "real_score_lora_ckpt", None) and getattr(args, "real_score_lora_base_model", None):
            print(f"Injecting LoRA into real_score {args.real_score_lora_base_model} with rank {getattr(args, 'real_score_lora_rank', 32)} and loading from {args.real_score_lora_ckpt}")
            real_score_lora_target_modules = getattr(args, "real_score_lora_target_modules", "q,k,v,o,ffn.0,ffn.2")
            real_score_lora_rank = getattr(args, "real_score_lora_rank", 32)
            
            # 获取要注入LoRA的模型部分
            real_score_base_model = getattr(self.real_score, args.real_score_lora_base_model)
            
            # 注入LoRA
            real_score_base_model = self._add_lora_to_model(
                real_score_base_model,
                target_modules=real_score_lora_target_modules.split(","),
                lora_rank=real_score_lora_rank
            )
            
            # 替换原模型
            setattr(self.real_score, args.real_score_lora_base_model, real_score_base_model)
            
            # 加载LoRA权重
            real_score_lora_state = load_state_dict(args.real_score_lora_ckpt)
            missing, unexpected = real_score_base_model.load_state_dict(real_score_lora_state, strict=False)
            print(f"Loaded real_score LoRA weights. Missing: {missing}, Unexpected: {unexpected}")
            
        # === LoRA 支持 for fake_score ===
        if getattr(args, "fake_score_lora_ckpt", None) and getattr(args, "fake_score_lora_base_model", None):
            print(f"Injecting LoRA into fake_score {args.fake_score_lora_base_model} with rank {getattr(args, 'fake_score_lora_rank', 32)} and loading from {args.fake_score_lora_ckpt}")
            fake_score_lora_target_modules = getattr(args, "fake_score_lora_target_modules", "q,k,v,o,ffn.0,ffn.2")
            fake_score_lora_rank = getattr(args, "fake_score_lora_rank", 32)
            
            # 获取要注入LoRA的模型部分
            fake_score_base_model = getattr(self.fake_score, args.fake_score_lora_base_model)
            
            # 注入LoRA
            fake_score_base_model = self._add_lora_to_model(
                fake_score_base_model,
                target_modules=fake_score_lora_target_modules.split(","),
                lora_rank=fake_score_lora_rank
            )
            
            # 替换原模型
            setattr(self.fake_score, args.fake_score_lora_base_model, fake_score_base_model)
            
            # 加载LoRA权重
            fake_score_lora_state = load_state_dict(args.fake_score_lora_ckpt)
            missing, unexpected = fake_score_base_model.load_state_dict(fake_score_lora_state, strict=False)
            print(f"Loaded fake_score LoRA weights. Missing: {missing}, Unexpected: {unexpected}")
    ...
    def _add_lora_to_model(self, model, target_modules, lora_rank, lora_alpha=None):
        """
        添加LoRA到模型，参考DiffusionTrainingModule的实现
        
        Args:
            model: 要添加LoRA的模型
            target_modules: LoRA目标模块列表
            lora_rank: LoRA rank
            lora_alpha: LoRA alpha (默认等于rank)
